...
Si on souhaite dépasser les endomorphismes de rang fini, il faut ajouter de la structure, typiquement de la topologie. Prenons l'exemple des espaces de Hilbert où les endomorphismes considérés sont les endomorphismes continus (ou opérateurs bornés) : pour un espace de Hilbert ${\mathcal H}$, on notera ${\mathcal B}({\mathcal H})$ l'ensemble de ces opérateurs. Si \(\mathcal H_1\) et \(\mathcal H_2\) sont deux espaces de Hilbert, leur produit tensoriel algébrique
\[
\mathcal H_1\otimes_{\mathrm{alg}}\mathcal H_2
\]
peut être muni du produit hermitien défini sur les tenseurs simples par
\[
\langle a_1\otimes a_2, b_1\otimes b_2\rangle
=
\langle a_1,b_1\rangle_{\mathcal H_1}
\langle a_2,b_2\rangle_{\mathcal H_2}.
\]

La complétion de l'espace ${\mathcal H}_1 \otimes_{\mathrm{alg}} {\mathcal H}_2$ (pour la norme hermmitienne induite) est un espace de Hilbert, noté ${\mathcal H}_1 \otimes {\mathcal H}_2$. Alors, l'espace ${\mathcal H} \otimes {\mathcal H}^*$ s'identifie à l'espace ${\mathcal B}_2({\mathcal H})$ des opérateurs de Hilbert-Schmidt : ce sont les opérateurs bornés $T : {\mathcal H} \rightarrow {\mathcal H}$ pour lesquels il existe une base hilbertienne $(e_n)$ telle que $\sum \limits_n \big\| T e_n \big\|^2 < +\infty$ (et alors cette condition ne dépend pas du choix de la base hilbertienne). Cependant, pour un opérateur de rang fini $T$, la trace est aussi donnée par la formule diagonale

$$\mathrm{Tr}(T) = \sum \limits_{n \in \mathbb{N}} \big \langle T e_n, e_n \big \rangle,$$

où $(e_n)$ est une base hilbertienne de ${\mathcal H}$. Cette série converge absolument dans le cas d'un opérateur de rang fini. Cette série peut être divergente pour certain opérateurs de ${\mathcal B}_2({\mathcal H})$, par exemple celui défini par $T e_n = \frac{1}{n} e_n$.

Ainsi, le produit tensoriel hilbertien mène naturellement aux opérateurs de Hilbert-Schmidt, mais cet espace est encore trop "gros" pour porter une trace au sens usuel. La bonne classe est celle des opérateurs de classe trace, notée $\mathcal{B}_1({\mathcal H})$ pour lesquels la formule
