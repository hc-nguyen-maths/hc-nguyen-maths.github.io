...
L'idée est qu'un $n$-uplet non ordonné de points de la sphère $\mathbb{S}^2$, avec multiplicités, peut être vu comme l'ensemble des racines (avec multiplicité) d'un polynôme complexe de degré au plus $n$, défini à multiplication près. Les coefficients de ce polynôme donnent alors des coordonnées projectives. Pouvoir identifier les singularités de $\mathrm{Conf}_n(\mathbb{S}^2)$ revient à une question simple, mais difficile à résoudre : peut-on reconnaître la présence de racines multiples et leur ordre de multiplicité en lisant suelement les coefficients de ce polynôme ?

## Bien au-delà de la musique

**La géométrie harmonique de Tymoczko.** Cette idée de représenter les accords comme des points d'un espace géométrique a été développée systématiquement par [Dmitri Tymoczko]{https://dmitri.mycpanel.princeton.edu/} dans ses travaux sur la géométrie des accords. Dans son approche, un accord est représenté par un espace géométrique singulier (un orbifold) et une progression harmonique devient alors un **chemin** dans cet espace. Les conduites de voix les plus efficaces - celles où chaque note bouge le moins possible - correspondent à des chemins courts, que l'on peut idéaliser géométriquement comme des géodésiques. Autrement dit, la géométrique de l'espace des accords ne sert pas seulement à classer les accords : elle permet aussi de mesurer la proximité entre deux accords et de comprendre pourquoi certaines progressions semblent particulièrement naturelles à l'oreille. Tymoczko formule précisément cette idée dans *The Geometry of Musical Chords*, où les accords sont modélisés comme des points d'orbifolds et le déplacements entre accords comme des chemins dans cet espace. 
