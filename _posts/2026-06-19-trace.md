...
Le déterminant des endomorphismes est parfois introduit par une propriété indépendante du choix d'une base : soit $V$ un espace vectoriel de dimension $n$, alors l'espace $\Lambda^n(V^*)$ des formes $n$-linéaires alternées est une droite vectorielle. En effet, si $f \in \Lambda^n(V^*)$, $(e_1, \ldots, e_n)$ est une base de $V$ et $x_1, \ldots, x_n \in V$, on écrit 
...
Ainsi, une forme $n$-linéaire alternée est entièrement déterminée par sa valeur sur une base. L'espace $\Lambda^n(V^*)$ est donc de dimension $1$. Cette observation permet de définir le déterminant sans partir des matrices. Si $u \in \mathrm{End}(V)$, alors $u$ agit naturellement sur $\Lambda^n(V^*)$ via 
$$u \cdot f : (x_1, \ldots, x_n) \in V^n \longmapsto f \big( u(x_1), \ldots, u(x_n) \big)$$
Comme $\Lambda^n(V^*)$ est une droite, cette action est la multiplication par un scalaire : ce scalaire est le déterminant de $u$.
...
$${\mathcal B}(V,W;Z) \cong {\mathcal L}(V \otimes W, Z),$$
où $\mathcal{B}(V,W;Z)$ désigne l'espace des applications bilinéaires $V \times W \rightarrow Z$.
